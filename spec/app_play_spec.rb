...
describe "The App" do
  ...
  context "Play" do
    ...
    it "should return a win for player 1" do
      ...
      expected_response = {"game":["X","X","X","","","","","",""], 'win': "Player 1 is Winner" }.to_json
      post "/play", @body1, @request_headers
      expect(last_response).to be_ok
      expect(last_response.body).to eq(expected_response)
    end
    ...
    it "should return a win for player 2" do
      ...
      @web_game.store({ @game_id => data })
      expected_response = { "game": ["O", "O", "O", "", "", "", "", "", ""], 'win': "Player 2 is Winner" }.to_json
      post "/play", @body2, @request_headers
      expect(last_response).to be_ok
      expect(last_response.body).to eq(expected_response)
    end
    ...
    it "should return a draw" do
      ...
      @web_game.store({ @game_id => data })
      expected_response = { "game": ["X", "X", "O", "O", "O", "X", "O", "X", "O"], 'draw': "Draw" }.to_json
      post "/play", @body2, @request_headers
      expect(last_response).to be_ok
      expect(last_response.body).to eq(expected_response)
    end
